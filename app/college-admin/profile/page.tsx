      ...
      <DashboardShell title="College Admin Profile">
        <div className="space-y-8 pb-12">
          ...
          <ProfileHeader
            ...
            roleTheme="college_admin"
            statusText="Active Admin"
            badges={badges}
            initials={initials}
            onEditProfile={() => {
              setEditForm({
          ...
          >
            <ProfileStatCard
              title="Admin Level"
              value="Full Admin"
              subtitle="Campus Tenant Authority"
              icon={Shield}
              variant="indigo"
            />
            <ProfileStatCard
              ...
              value="Verified"
              subtitle="Active Tenant Member"
              icon={CheckCircle2}
              variant="emerald"
            />
            <ProfileStatCard
              title="Subscription Plan"
              value={(college?.plan || "Enterprise").toUpperCase()}
              subtitle="Full Suite Enabled"
              icon={Star}
              variant="purple"
            />
            <ProfileStatCard
              title="Subdomain Handle"
              value={college?.subdomain || "demo"}
              subtitle=".campusos.com"
              icon={Building2}
              variant="amber"
            />
          </motion.div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {/* Left 2 Cols: Personal Info & Privileges */}
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Card className="border-border/60 bg-card/60 backdrop-blur-md shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg font-bold flex items-center gap-2">
                    <User className="h-5 w-5 text-indigo-500" /> Administrator Account Info
                  </CardTitle>
              ...
              <Card className="border-border/60 bg-card/60 backdrop-blur-md shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg font-bold flex items-center gap-2">
                    <Shield className="h-5 w-5 text-indigo-500" /> Active Admin Privileges
                  </CardTitle>
                  <CardDescription>Assigned tenant access rights</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-3 sm:grid-cols-3">
                    {[
                      ...
                      <div
                        key={privilege}
                        className="flex items-center gap-2.5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4"
                      >
            ...
            {/* Right Col: Institution & Activity */}
            <motion.div
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <Card className="border-border/60 bg-card/60 backdrop-blur-md shadow-sm">
                ...
                <CardContent className="space-y-3">
                  <div className="p-4 rounded-2xl border border-border/60 bg-background/50">
                    <p className="text-xs text-muted-foreground font-medium">College Name</p>
                    <p className="text-sm font-bold text-foreground mt-0.5">{college?.name || "CampusOS Institution"}</p>
                  </div>
                  <div className="p-4 rounded-2xl border border-border/60 bg-background/50">
                    <p className="text-xs text-muted-foreground font-medium">Domain URL</p>
              ...
              <Card className="border-border/60 bg-card/60 backdrop-blur-md shadow-sm">
                <CardHeader>
                  <CardTitle className="text-lg font-bold flex items-center gap-2">
                    <Activity className="h-5 w-5 text-indigo-500" /> Security Log
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="relative pl-6 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-border space-y-4">
                    <div className="relative">
                      ...
                      <p className="text-xs font-bold text-foreground">Admin Session Verified</p>
        ...
        <ProfileModal
          ...
          title="Edit College Admin Profile"
